...
if [[ $# -lt 4 || $# -gt 5 ]]; then
  print -u2 "usage: $0 <status-file> <expected-status> <delivery-file> <completion-marker> [timeout-seconds>=1]"
  exit 64
fi
...
while true; do
  ...
  if [[ "$last_status" == "$expected_status" && -f "$delivery_file" ]]; then
    ...
    if [[ "$last_line" == "$completion_marker" ]]; then
      printf 'TASK_DELIVERY_COMPLETE\t%s\n' "$delivery_file"
      exit 0
    fi
  fi

  if (( SECONDS >= deadline )); then
    printf 'TASK_WAIT_TIMEOUT\tstatus=%s\ttimeout=%s\n' "$last_status" "$timeout_seconds" >&2
    exit 124
  fi
