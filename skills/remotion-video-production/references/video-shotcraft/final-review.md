...
> **Akasha 快照适配：** 本文中的 `gallery/api/library.json` 映射为
> `assets/video-shotcraft/library.json`，`references/shots/` 映射为
> `references/video-shotcraft/shots/`，`demos/` 映射为
> `assets/video-shotcraft/demos/`。产品定位、功能覆盖、真实页面等检查只对
> 产品片适用；文章、照片编排和通用动态图形以当前 brief 的核心信息、素材
> 边界和视觉合同替代，产品专属条目标记为 N/A，不单独构成失败。
