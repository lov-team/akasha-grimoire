...
test_root="$(mktemp -d /tmp/codex-cli-submit-test.XXXXXX)"
...
"$submit_script" '=codex-test:0.0' "$input_file"

expected_log="$test_root/expected.log"
buffer_name="$(awk '$1 == "load-buffer" { print $3 }' "$TMUX_TEST_LOG")"
[[ -n "$buffer_name" ]]
cat > "$expected_log" <<EOF
list-panes -t =codex-test:0.0 -F #{session_name}:#{window_index}.#{pane_index}
load-buffer -b $buffer_name $input_file
paste-buffer -b $buffer_name -t =codex-test:0.0 -d
send-keys -t =codex-test:0.0 Enter
EOF
diff -u "$expected_log" "$TMUX_TEST_LOG"

print -r -- $'第一行\n第二行' > "$input_file"
: > "$TMUX_TEST_LOG"
if "$submit_script" '=codex-test:0.0' "$input_file" >/dev/null 2>&1; then
