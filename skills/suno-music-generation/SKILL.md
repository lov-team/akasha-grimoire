...
如果提交阶段已经返回 task id，后续恢复工作必须继续 fetch 这个 task id，不得因为轮询中断、界面切换或主 Agent 续接而重复 submit。提交成功不等于歌曲生成成功，生成成功不等于音频下载成功，下载成功也不等于试听验收通过；四个状态必须分别记录。
...
讲解版与纯 MV 版的配乐目标必须分开管理。讲解版优先保留旁白可懂度，并记录音乐候选、裁切区间、淡出尾巴和混音电平；纯 MV 版才以完整歌曲结构和音乐高潮为主，不得把同一混音直接覆盖两个版本。
